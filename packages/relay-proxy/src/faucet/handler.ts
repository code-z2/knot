...
import type { Env, FaucetFundRequestModel, SupportMode } from '../shared/types';
import { normalizeAddress } from '../shared/validation';

export async function handleFaucetFund(
    rawBody: string,
    env: Env,
    ...
    await faucetKV.put(fundingKey, JSON.stringify({ state: 'pending', updatedAt: Date.now() }), {
...
function parseFaucetFundRequest(rawBody: string): FaucetFundRequestModel {
    ...
    if (!SUPPORT_MODES.has(supportMode as SupportMode)) {
...
async function readFaucetFundingState(
    ...
    try {
        ...
        if (parsed.state === 'pending') return 'pending';
        if (parsed.state === 'funded') return 'funded';
    } catch {
