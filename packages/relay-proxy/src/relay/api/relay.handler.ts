...
import { BadRequestError, PaymentRequiredError } from '../../shared/errors';
import { jsonResponse } from '../../shared/http';
import type { Env, SupportMode } from '../../shared/types';
import { formatNativeToken, normalizeAddress } from '../../shared/validation';

import { getRelayStatus, quoteTotalWei, sendRelayTransaction } from '../services/gelato.service';
import {
    assertRelayTransactionsMatchAccount,
    parseSubmitRequest,
    sanitizePaymentOptions,
} from './relay.parser';
...
import type { RelaySubmissionModel } from '../types';

/**
 * Core ingress handler for POST /v1/relay/submit.
 * Parses, validates signatures, debits the gas tank, and dispatches transactions.
 */
export async function handleSubmitRelay(rawBody: string, env: Env): Promise<Response> {
    ...
    // Optimistic accounting: Reserve/debit the estimate immediately.
    ...
    const immediateResults = await Promise.allSettled(
        body.immediateTxs.map(async (tx) => {
            const result = await sendRelayTransaction(
...
/** Unified relay status lookup for both Gelato and deferred IDs. */
export async function handleRelayStatus(url: URL, env: Env): Promise<Response> {
    ...
    if (!id) throw new BadRequestError('Missing relay task id.');
    if (!SUPPORT_MODES.has(modeRaw as SupportMode))
        throw new BadRequestError('Missing or invalid supportMode.');

    const status = await getRelayStatus(id, modeRaw as SupportMode, env);
    return jsonResponse({ ok: true, status });
}

/** Read-only gas tank balance endpoint. */
...
function assertSingleSupportModeInvocation(
    txs: ReadonlyArray<{ supportMode?: SupportMode }>,
    ...
    for (const tx of txs) {
        ...
        if (invocationMode !== mode) {
            throw new BadRequestError(
                'Mixed support modes in one relay invocation are not allowed.',
            );
        }
    }
}
