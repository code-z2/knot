...
import { ERC20_TRANSFER_ABI } from '../shared/abis/erc20';
import { FAUCET_CHAINS } from '../shared/chains';
import { ETH_DRIP_WEI, TESTNET_USDC_BY_CHAIN, USDC_DRIP_AMOUNT } from '../shared/constants';
...
export class FaucetTracker extends DurableObject<Env> {
    async fetch(request: Request): Promise<Response> {
        ...
        const faucetPrivateKey = normalizePrivateKey(SERVER_KEY);
    ...
    private async fundOnChain(
        ...
        if (usdcAddress) {
            try {
                ...
                const reason =
                    error instanceof Error ? error.message : 'unknown usdc transfer error';
