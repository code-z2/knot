...
public enum SmartAccount {
    ...
    public enum Initialize {
        ...
        public static func asCall(
            ...
            try Call(
                to: account,
                dataHex: "0x" + encodeCall(passkeyPublicKey: passkeyPublicKey, config: config, initSignature: initSignature).toHexString(),
                valueWei: "0",
            )
        }
    }
    ...
    public enum AccumulatorFactory {
        private static let accumulatorCreationCode: Data = {
            guard
                let bytecode = Data.fromHex(
                    AccumulatorCreationCode.hex.replacingOccurrences(of: "0x", with: ""),
                )
            else {
                preconditionFailure("Accumulator creation code is invalid.")
            }
            return bytecode
        }()
        ...
        public static func deriveAddress(
            userAccount: String,
            accumulatorFactory: String = AAConstants.accumulatorFactoryAddress,
        ) throws -> String {
            let userAddress = try AAUtils.addressData(userAccount)
            let factoryAddress = try AAUtils.addressData(accumulatorFactory)
            let salt = Data(userAddress.sha3(.keccak256))
            let constructorArgs = try ABIWord.address(userAccount)
            let initCodeHash = Data((accumulatorCreationCode + constructorArgs).sha3(.keccak256))
            let digest = Data((Data([0xFF]) + factoryAddress + salt + initCodeHash).sha3(.keccak256))
            return "0x" + digest.suffix(20).toHexString()
        }
    }
...
public actor SmartAccountClient {
    ...
    public func computeAccumulatorAddress(
        ...
        _ = chainId
        return try SmartAccount.AccumulatorFactory.deriveAddress(userAccount: account)
    }

    public nonisolated func computeAccumulatorAddress(account: String) throws -> String {
        try SmartAccount.AccumulatorFactory.deriveAddress(userAccount: account)
    }
