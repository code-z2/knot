...
final class AATests: XCTestCase {
        ...
        let expected = try Data(
            (
                ABIWord.uint(BigUInt(chainId))
                    + (ABIWord.address(account))
                    + (ABIWord.bytes32(passkey.x))
                    + (ABIWord.bytes32(passkey.y))
                    + (ABIWord.address(config.accumulatorFactory))
                    + (ABIWord.address(config.wrappedNativeToken))
                    + (ABIWord.address(config.spokePool))
            ).sha3(.keccak256),
        )
    ...
    func testDeterministicAccumulatorAddressDerivationMatchesKnownVectors() throws {
        let client = SmartAccountClient()

        XCTAssertEqual(
            try client.computeAccumulatorAddress(account: "0x000000000000000000000000000000000000a11c"),
            "0xbe81a64b971599bf52304a2c352defb9cf38bafb",
        )
        XCTAssertEqual(
            try client.computeAccumulatorAddress(account: "0x0000000000000000000000000000000000000acc"),
            "0xed78516b4c1148fb9f477aeb10433d4519548e23",
        )
        XCTAssertEqual(
            try client.computeAccumulatorAddress(account: "0x400f00000000000000000000000000000000df5a"),
            "0x6266685fd2ec73d6fd8fb5c63fdf2c7d37022150",
        )
    }
