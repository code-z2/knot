...
final class AAExecutionService {
    ...
    private func buildExecutionRequest(
        ...
        let gasLimit: String = if skipPreflight {
            defaultGasLimitHex
        } else {
            try await estimateGasWithFallback(
                chainId: chainId,
                from: context.account.eoaAddress,
